...
## Helper function checking whether all of the vertices have been visited
...
def traveling_salesperson(graph):
  # Initialize by setting all vertices to be "unvisited" and randomly select start vertex 
  ts_path = ""
  visited_vertices = {x: "unvisited" for x in graph.graph_dict}
  current_vertex = random.choice(list(graph.graph_dict))
  visited_vertices[current_vertex] = "visitied"
  ts_path += current_vertex

  visited_all_vertices = visited_all_nodes(visited_vertices) # returns False
  while not visited_all_vertices:
    current_vertex_edges = graph.graph_dict[current_vertex].get_edges() # return list of current_vertex's edge vertices
    current_vertex_edge_weights = {}

    # getting current vertex's edges
    ...
    while not found_next_vertex:
      if current_vertex_edge_weights is None:
        break
      # Setting Next vertex (min among edges)
